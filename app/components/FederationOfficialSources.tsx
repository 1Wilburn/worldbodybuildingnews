...
export default function FederationOfficialSources(
  ...
  if (fed.official_url) rows.push({ label: 'Official Website', href: fed.official_url });
  if (fed.rules_url) rows.push({ label: 'Rules & Divisions', href: fed.rules_url });
  if (fed.calendar_url) rows.push({ label: 'Competition Calendar', href: fed.calendar_url });
  if (fed.registration_url) rows.push({ label: 'Athlete Registration', href: fed.registration_url });

  const ig = norm(fed.instagram);
  const tw = norm(fed.x_twitter);
  const yt = norm(fed.youtube);
